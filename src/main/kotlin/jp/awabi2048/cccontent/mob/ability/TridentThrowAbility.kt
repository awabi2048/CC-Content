...
open class TridentThrowAbility(
    ...
    override fun onTick(context: MobRuntimeContext, runtime: MobAbilityRuntime?) {
        ...
        val homing = homingConfig
        if (homing != null) {
            HomingArrowService.getInstance(context.plugin).launch(
                HomingArrowService.LaunchSpec(
                    arrowId = projectile.uniqueId,
                    targetId = target.uniqueId,
                    accuracy = homing.accuracy,
                    turnStrength = homing.turnStrength,
                    maxTurnDegrees = homing.maxTurnDegrees
                )
            )
        }
        ...
        entity.world.playSound(entity.location, Sound.ENTITY_DROWNED_SHOOT, 0.85f, 1.0f)
        entity.world.playSound(entity.location, Sound.ITEM_TRIDENT_THROW, 0.9f, 1.0f)
    ...
    protected open fun loaderKeyString(): String {
        return "trident_throw_loader_$id"
    }
