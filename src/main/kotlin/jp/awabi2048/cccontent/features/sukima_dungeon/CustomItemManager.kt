...
object CustomItemManager {
    ...
    fun applyBookmarkMetadata(item: ItemStack, player: Player?, tier: DungeonTier) {
        ...
        meta.lore(CCSystem.getAPI().getLoreService().render(GuiLoreSpec.Rich(MessageManager.getList(player, "item_bookmark_lore").map { GuiLoreLine.Text(it) }, GuiLoreFrame.NONE)))
    ...
    fun applyTalismanMetadata(item: ItemStack, player: Player?) {
        ...
        meta.lore(CCSystem.getAPI().getLoreService().render(GuiLoreSpec.Rich(MessageManager.getList(player, "item_talisman_lore").map { GuiLoreLine.Text(it) }, GuiLoreFrame.NONE)))
        meta.addItemFlags(ItemFlag.HIDE_ATTRIBUTES, ItemFlag.HIDE_ENCHANTS)
