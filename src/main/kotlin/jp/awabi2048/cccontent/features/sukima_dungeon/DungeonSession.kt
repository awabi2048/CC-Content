...
object DungeonSessionManager {
    ...
    fun endSession(playerUUID: UUID) {
        if (sessions.remove(playerUUID) != null) {
            markDirty()
        }
    }
    ...
    fun flushIfDirty(plugin: JavaPlugin) {
        if (!dirty) {
            return
        }
        saveSessions(plugin)
    ...
    fun saveSessions(plugin: JavaPlugin) {
        val file = File(plugin.dataFolder, "data/sukima_dungeon/sessions.yml")
        ...
        for (session in sessions.values) {
            ...
            val minibossList = session.minibossMarkers.map {
                "${it.key.first},${it.key.second}|${it.value.world?.name ?: ""},${it.value.x},${it.value.y},${it.value.z},${it.value.yaw},${it.value.pitch}"
            }
            ...
            val spawnedMobsList = session.spawnedMobs.map {
                "${it.key.world?.name ?: ""},${it.key.x},${it.key.y},${it.key.z},${it.key.yaw},${it.key.pitch}|${it.value}"
            }
    ...
    fun loadSessions(plugin: JavaPlugin) {
        ...
        val sessionsSection = config.getConfigurationSection("sessions") ?: run {
            dirty = false
            return
        }
        ...
        for (uuidString in sessionsSection.getKeys(false)) {
            ...
            val tier = runCatching {
                DungeonTier.valueOf(config.getString("$path.tier") ?: "BROKEN")
            }.getOrDefault(DungeonTier.BROKEN)
            ...
            val restCells = config.getStringList("$path.restCells").mapNotNull {
                val parts = it.split(",")
                if (parts.size == 2) parts[0].toIntOrNull()?.let { x -> parts[1].toIntOrNull()?.let { z -> x to z } } else null
            }.toSet()

            val minibossMarkers = config.getStringList("$path.minibossMarkers").mapNotNull {
                val parts = it.split("|")
                ...
                val cellX = cellParts[0].toIntOrNull() ?: return@mapNotNull null
                val cellZ = cellParts[1].toIntOrNull() ?: return@mapNotNull null
                val world = Bukkit.getWorld(locParts[0]) ?: return@mapNotNull null
                val loc = Location(
                    world,
                    locParts[1].toDoubleOrNull() ?: return@mapNotNull null,
                    locParts[2].toDoubleOrNull() ?: return@mapNotNull null,
                    locParts[3].toDoubleOrNull() ?: return@mapNotNull null,
                    locParts[4].toFloatOrNull() ?: return@mapNotNull null,
                    locParts[5].toFloatOrNull() ?: return@mapNotNull null
                )
                (cellX to cellZ) to loc
            }.toMap()

            val minibossTriggered = config.getStringList("$path.minibossTriggered").mapNotNull {
                val parts = it.split(",")
                if (parts.size == 2) {
                    parts[0].toIntOrNull()?.let { x -> parts[1].toIntOrNull()?.let { z -> x to z } }
                } else {
                    null
                }
            }.toMutableSet()
            ...
            val spawnedMobs = config.getStringList("$path.spawnedMobs").mapNotNull {
                val parts = it.split("|")
                if (parts.size != 2) return@mapNotNull null
                ...
                val mobUuid = runCatching { UUID.fromString(parts[1]) }.getOrNull() ?: return@mapNotNull null
                loc to mobUuid
            }.toMap().toMutableMap()
            ...
            val session = DungeonSession(
                ...
                collectedSprouts = collectedSprouts,
                totalSprouts = totalSprouts,
            ...
            worldName?.let { world ->
                if (Bukkit.getWorld(world) == null) {
                    DungeonManager.loadDungeonWorld(world, themeName)
                }
            }
        }
        dirty = false
