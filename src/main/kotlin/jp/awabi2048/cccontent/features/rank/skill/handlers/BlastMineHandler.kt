...
class BlastMineHandler(
    ...
    override fun applyEffect(context: EffectContext): Boolean {
        ...
        applyDeferredDurabilityAtStart(player, playerUuid, originHeldSlot, totalBlocks)
    ...
    private fun applyDeferredDurabilityAtStart(player: Player, playerUuid: UUID, originHeldSlot: Int, plannedBreaks: Int) {
        if (plannedBreaks <= 0 || originHeldSlot !in 0..8) return

        val item = player.inventory.getItem(originHeldSlot) ?: return
        if (item.type.maxDurability <= 0 || item.type.isAir) return

        val additionalDamage = calculateDeferredDurabilityDamage(playerUuid, item, plannedBreaks)
