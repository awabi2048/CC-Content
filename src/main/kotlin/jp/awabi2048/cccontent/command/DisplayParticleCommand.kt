...
class DisplayParticleCommand(private val plugin: Plugin) {
    fun execute(sender: CommandSender, args: Array<String>): Boolean {
        ...
        val request = runCatching {
            val preset = CCSystem.getAPI().getDisplayEffectService().listDisplayParticlePresets()
                .firstOrNull { it.id.value == parsed.patternId }
                ?: throw IllegalArgumentException("未登録の外観プリセットです: ${parsed.patternId}")
            DisplayParticleEmissionRequest(
                presetId = DisplayParticlePresetId(parsed.patternId),
                motionPresetId = preset.defaultMotionPresetId,
                collisionMode = preset.defaultCollisionMode,
                delta = parsed.delta,
                speed = parsed.speed,
                count = parsed.count,
...
internal object DisplayParticleCommandSyntax {
    fun parse(args: Array<String>): ParsedDisplayParticleCommand {
        ...
        val patternId = namespaced(args[0])
